defmodule GuildshipWeb.UserAuthTest do
  use GuildshipWeb.ConnCase, async: true

  alias Guildship.Accounts
  alias GuildshipWeb.UserAuth
  import Guildship.AccountsFixtures

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, GuildshipWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
